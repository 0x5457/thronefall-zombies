...
export function addOcclusionSilhouette(character: T.Object3D): void {
  ...
  character.traverse((part) => {
    if ((part as T.Mesh).isMesh) parts.push(part as T.Mesh);
  });
  for (const part of parts) {
    ...
    part.material = (part.material as T.Material).clone();
