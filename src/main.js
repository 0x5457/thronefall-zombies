...
import { makeWorld, character, structure, walkable, seeded, enemyModel, mesh, enemyMaterials } from './world.js';
...
import {
  COSTS, NAMES, newGame, canBuild, buy, damageCamp, overlaps, PERKS, attackDamage, logYield, DAY_LENGTH,
  ENEMY_TYPES, WAVES, WEAPONS, EXPEDITIONS, LANES, applyArmor, DASH, FLARE, MEDKIT_HEAL, MAX_LEVEL, REPAIR_WOOD,
  equipWeapon, unlockWeapon, expedition, heal, damagePlayer, playerDown, tickSurvival, upgradeCost, refundValue, upgrade, repair, wavePlan, maxHp, useFlare,
  siegeGoal, rangedGoal, revealed, hasFurniture, rvSlots, weaponDamage, weaponRange, maxMedkits, dawnRating,
} from './rules.js';
...
const moon = new T.DirectionalLight('#9fc4e8', 0); moon.position.set(24, 30, 20); scene.add(moon, moon.target);
const sky = new T.HemisphereLight('#d4e2e2', '#92906f', 2.1); scene.add(sky);
const fill = new T.AmbientLight('#b0c7b8', .28); scene.add(fill);
const playerLight = new T.PointLight('#ffd9a0', 0, 8, 1.5); scene.add(playerLight);
const muzzleLight = new T.PointLight('#ffd08a', 0, 6.5, 1.6); scene.add(muzzleLight);
const world = makeWorld(scene);
setLoadingProgress(32, '营地材质已就绪');
const motionPreference = matchMedia('(prefers-reduced-motion: reduce)');
let ambientTime = 0;
const state = newGame();
world.logs.forEach((log, index) => { log.id = `log-${index + 1}`; });
...
const buildings = [], enemies = [], enemyPools = {}, shots = [], particles = [], keys = new Set();
...
function attachBuildingView(b, mesh, light = null) {
  ...
  scene.add(mesh); if (light) scene.add(light);
}
function detachBuildingView(b) {
  const view = buildingViews.get(b.id); if (!view) return;
  scene.remove(view.mesh); if (view.light) scene.remove(view.light);
  buildingViews.delete(b.id);
}
function syncBuildingScene() {
  for (const view of buildingViews.values()) { scene.remove(view.mesh); if (view.light) scene.remove(view.light); }
  buildingViews.clear();
  for (const b of buildings) {
    const m = structure(b.type, b.level);
    m.position.set(b.x, 0, b.z); m.rotation.y = b.angle || 0; m.userData.building = b;
    let light = null;
    if (b.type === 'lantern') { light = new T.PointLight('#ffcf7e', 0, 9, 1.5); light.position.set(b.x, 2.6, b.z); }
    attachBuildingView(b, m, light);
    const view = viewOf(b); view.growth = 1;
  }
}
...
let daylight = 1, time = 0, last = performance.now(), zoom = 23, shotTimer = 0, remaining = 0, toastTimer, hurtTimer;
let movementTarget = null, photoMode = false, homeMode = false, nightClock = 0, spawnQueue = [], dash = null, playerInvuln = 0;
// Per-night telemetry feeds the dawn rating; it never changes combat rules.
let nightRun = { startHealth: 100, clearHealth: 100, downs: 0, buildingsLost: 0, seconds: 0 }, lastRating = null;
let muzzleFlashes = 0, hitFlashes = 0, deaths = 0, knockbacks = 0, recoils = 0, spitCount = 0;
const trauma = createTrauma();
const hitStop = createHitStop();
const enemyFlashMaterial = new T.MeshStandardMaterial({ color: '#ffdcae', flatShading: true, roughness: 1 });
const SHAKE_STORAGE = 'pinefall.feel.v1';
let shakeEnabled = !motionPreference.matches;
try { const saved = localStorage.getItem(SHAKE_STORAGE); if (saved !== null) shakeEnabled = saved === 'true'; } catch { /* storage may be blocked */ }
...
for (const [type, count] of Object.entries(POOL_SIZE)) {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const record = { mesh: model, type, alive: false, hp: 0, maxHp: 0, attack: 0, slow: 0, slowFactor: 1, seed: random() * 20, kx: 0, kz: 0 };
    pool.push(record); enemies.push(record);
  }
...
const muzzles = [], muzzleGeometry = new T.OctahedronGeometry(.5, 0);
const muzzleMaterial = new T.MeshBasicMaterial({ color: '#ffe7ab', transparent: true, opacity: .92, blending: T.AdditiveBlending, depthWrite: false });
for (let i = 0; i < 14; i++) {
  const m = new T.Mesh(muzzleGeometry, muzzleMaterial); m.visible = false; scene.add(m);
  muzzles.push({ mesh: m, life: 0 });
}
// Corrosive spit from the ranged enemy: a visible arc gives the player time to dodge.
const spits = [], spitMaterial = new T.MeshBasicMaterial({ color: '#b6d06a', transparent: true, opacity: .92, blending: T.AdditiveBlending, depthWrite: false });
for (let i = 0; i < 24; i++) {
  const m = new T.Mesh(new T.IcosahedronGeometry(.15, 0), spitMaterial); m.visible = false; scene.add(m);
  spits.push({ mesh: m, life: 0, total: 1, kind: '', ref: null, damage: 0, from: new T.Vector3(), to: new T.Vector3() });
}
...
function flareVisual() {
  ...
  if (!flare) {
    ...
    const orb = mesh(group, new T.SphereGeometry(.26, 8, 6), '#ffc07a', 0, .6, 0, 1, 1, 1, true);
    const ring = new T.Mesh(new T.RingGeometry(FLARE.radius - .2, FLARE.radius, 44), new T.MeshBasicMaterial({ color: '#ff9a52', transparent: true, opacity: .35, side: T.DoubleSide, depthWrite: false }));
    ring.rotation.x = -Math.PI / 2; ring.position.y = .03; group.add(ring);
    const light = new T.PointLight('#ff9a52', 0, 15, 1.5); light.position.y = 1.2; group.add(light);
...
const selectionRing = new T.Mesh(new T.RingGeometry(1.05, 1.24, 30), new T.MeshBasicMaterial({ color: '#f0d59a', transparent: true, opacity: .75, side: T.DoubleSide, depthWrite: false }));
selectionRing.rotation.x = -Math.PI / 2; selectionRing.position.y = .045; selectionRing.visible = false; scene.add(selectionRing);
...
function flashHurt() {
  ...
  const el = $('#hurt-flash'); el.classList.add('show');
  clearTimeout(hurtTimer); hurtTimer = setTimeout(() => el.classList.remove('show'), 90);
}
...
$('#shake').addEventListener('click', () => {
  ...
  try { localStorage.setItem(SHAKE_STORAGE, String(shakeEnabled)); } catch { /* no persistence */ }
  syncShakeUI(); toast(shakeEnabled ? '镜头震动已开启。' : '镜头震动已关闭。');
});
syncShakeUI();
function setEnemyMaterial(enemy, material) {
  enemy.mesh.traverse(part => { if (part.isMesh && part.material !== enemyMaterials.glow) part.material = material; });
}
function syncPlayerHud() {
  const max = maxHp(state), hp = Math.max(0, state.playerHp);
  ...
  if (state.flareCooldown <= 0) { flare.textContent = '✦ 就绪'; flare.className = 'ready'; }
  else { flare.textContent = `✦ ${Math.ceil(state.flareCooldown)}s`; flare.className = 'cooling'; }
}
function syncUI() {
  ...
  $('#wood').textContent = state.wood; $('#scrap').textContent = state.scrap; $('#health').textContent = Math.ceil(state.health); $('#kills').textContent = state.kills;
  ...
  $('#note-body').textContent = state.phase === 'day' ? '在空地建起防线。按 Tab 打开营地手册：工坊、远征与战前情报。' : `第 ${state.day} 夜 · 自动射击已就绪，守住房车直到黎明。`;
...
function togglePause() {
  ...
  keys.clear(); movementTarget = null;
}
function openPerks() {
  ...
  const rating = dawnRating({ campLost, downs: nightRun.downs, buildingsLost: nightRun.buildingsLost, nightSeconds: nightRun.seconds });
  lastRating = { ...rating, day: state.day - 1, campLost, downs: nightRun.downs, buildingsLost: nightRun.buildingsLost, seconds: Math.round(nightRun.seconds) };
  $('#perk-summary').textContent = `第 ${state.day - 1} 夜已守住 · 评分 ${rating.grade}（${rating.score}）· 击退 ${state.kills} · 营地 −${campLost}% · 耗时 ${Math.round(nightRun.seconds)} 秒。选择期间不消耗准备时间。`;
...
$('#perk-dialog').addEventListener('close', () => { if (state.perkPending) openPerks(); });
...
function place() {
  ...
  m.position.set(p.x, 0, p.z); m.rotation.y = ghostAngle; m.scale.setScalar(.01);
  const building = { id: state.nextBuildId++, type, x: p.x, z: p.z, angle: ghostAngle, r: type === 'tower' ? 1.2 : type === 'fence' ? 1 : .35, level: 1, maxHp: type === 'fence' ? 150 : 220, hp: type === 'fence' ? 150 : 220, invested: { wood: COSTS[type], scrap: 0 } };
  let light = null;
  if (type === 'lantern') { light = new T.PointLight('#ffcf7e', 0, 9, 1.5); light.position.set(p.x, 2.6, p.z); }
...
const towerStats = b => ({ damage: 2 + (b.level - 1), range: 12 + (b.level - 1) * 1.6, cooldown: Math.max(.55, .9 - (b.level - 1) * .12) });
const lanternRadius = b => 5 + (b.level - 1) * 1.4;
const lanternSlow = b => Math.max(.4, .55 - (b.level - 1) * .07);
function refitBuilding(b) {
  const view = viewOf(b);
  const m = structure(b.type, b.level);
  m.position.set(b.x, 0, b.z); m.rotation.y = view.mesh.rotation.y; m.scale.setScalar(.6); m.userData.building = b;
  scene.remove(view.mesh); view.mesh = m; view.growth = .4; scene.add(m);
}
function renderBuildingPanel() {
  const panel = $('#building-panel'), b = selectedBuilding;
  if (!b || !buildings.includes(b)) { panel.hidden = true; return; }
  panel.hidden = false;
  const day = state.phase === 'day';
  const cost = upgradeCost(b), refund = refundValue(b);
  ...
  const stats = b.type === 'tower' ? `伤害 ${towerStats(b).damage} · 射程 ${towerStats(b).range}` : b.type === 'lantern' ? `减速半径 ${lanternRadius(b).toFixed(1)}` : `阻挡单路 · 为火力争取时间`;
  $('#building-stats').textContent = `${stats} · 耐久 ${Math.ceil(b.hp)} / ${b.maxHp}`;
  const upgradeBtn = $('#building-upgrade');
  upgradeBtn.textContent = b.level >= MAX_LEVEL ? '已满级' : `升级 Lv.${b.level + 1} · ▰${cost.wood} ⚙${cost.scrap}`;
  upgradeBtn.disabled = b.level >= MAX_LEVEL || !day || state.wood < cost.wood || state.scrap < cost.scrap;
...
function selectBuilding(b) {
  ...
  if (selectedBuilding) {
    selectionRing.position.set(selectedBuilding.x, .045, selectedBuilding.z);
    selectBuild(null);
  }
  renderBuildingPanel();
}
function buildingAt(object) {
  let node = object;
  while (node) { if (node.userData?.building) return node.userData.building; node = node.parent; }
  return null;
}
function upgradeBuilding() {
  const b = selectedBuilding; if (!b) return;
  if (!upgrade(state, b)) { toast(state.phase === 'night' ? '夜间无法施工。' : '资源不足，无法升级。'); return; }
  refitBuilding(b); burst(b.x, 1, b.z, 18); audio.effect('build');
  toast(`${NAMES[b.type]}升级至 Lv.${b.level}`); renderBuildingPanel(); syncUI();
}
function repairBuilding() {
  const b = selectedBuilding; if (!b) return;
  if (!repair(state, b)) { toast(state.phase === 'night' ? '夜间无法维修。' : '木材不足或结构完好。'); return; }
  burst(b.x, .7, b.z, 10); audio.effect('build'); toast(`${NAMES[b.type]}维修完成 · −${REPAIR_WOOD} 木材`); renderBuildingPanel(); syncUI();
}
function dismantleBuilding() {
  const b = selectedBuilding; if (!b) return;
  if (state.phase !== 'day') { toast('夜间无法拆除。'); return; }
  const refund = refundValue(b);
  state.wood += refund.wood; state.scrap += refund.scrap;
  detachBuildingView(b);
  buildings.splice(buildings.indexOf(b), 1);
  burst(b.x, .7, b.z, 22); audio.effect('destroy');
  toast(`已拆除 ${NAMES[b.type]} · 返还 ▰${refund.wood} ⚙${refund.scrap}`);
  selectBuilding(null); syncUI();
}
...
canvas.addEventListener('pointerdown', event => {
  ...
  updatePointer(event);
  if (selected) { updateGhost(); place(); return; }
  const hit = raycaster.intersectObjects(buildings.map(b => viewOf(b).mesh), true)[0];
  const building = hit ? buildingAt(hit.object) : null;
  if (building) { selectBuilding(building); return; }
  selectBuilding(null);
  if (walkable(cursor.x, cursor.z)) movementTarget = cursor.clone();
});
canvas.addEventListener('contextmenu', event => { event.preventDefault(); selectBuild(null); selectBuilding(null); });
...
function enterOrExit() {
  ...
  selectBuild(null); selectBuilding(null); movementTarget = null; keys.clear(); pointerInside = false;
...
function switchRV() {
  ...
  $('#rv-door').hidden = true;
  if (inside) { rv.render(); $('#leave-rv').focus(); } else $('#world').focus();
  resize(); syncUI();
}
...
function beginNight() {
  spawnQueue = wavePlan(state.day); remaining = spawnQueue.length; nightClock = 0;
  nightRun = { startHealth: state.health, clearHealth: state.health, downs: 0, buildingsLost: 0, seconds: 0 };
...
function transition() {
  ...
  if (phaseValue(actor.getSnapshot()) === 'night') {
    ...
    nightRun.clearHealth = state.health; nightRun.seconds = nightClock;
...
function openHelp() { actor.send({ type: 'OPEN_HELP' }); $('#help-dialog').showModal(); keys.clear(); }
...
function manualCard({ title, tag, body, stats, action, actionText, disabled, disabledText, owned }) {
  const card = document.createElement('section'); card.className = `manual-card${owned ? ' owned' : ''}`;
  const head = document.createElement('div'); head.className = 'card-head';
  const name = document.createElement('strong'); name.textContent = title;
  const label = document.createElement('span'); label.className = 'tag'; label.textContent = tag;
  head.append(name, label); card.append(head);
  const text = document.createElement('p'); text.textContent = body; card.append(text);
  if (stats) { const grid = document.createElement('div'); grid.className = 'manual-stat-grid'; stats.forEach(s => { const item = document.createElement('span'); item.textContent = s; grid.append(item); }); card.append(grid); }
  if (actionText) {
    const button = document.createElement('button'); button.className = action ? 'primary' : 'ghost'; button.textContent = actionText;
    button.disabled = !!disabled; if (action) button.addEventListener('click', action);
    card.append(button);
  }
  if (disabled && disabledText) { const note = document.createElement('p'); note.textContent = disabledText; card.append(note); }
  return card;
}
function renderManual() {
  document.querySelectorAll('[data-manual-tab]').forEach(tab => tab.setAttribute('aria-selected', String(tab.dataset.manualTab === manualTab)));
  const page = $('#manual-page'), cards = [];
  if (manualTab === 'workshop') {
    const bench = hasFurniture(state, 'workbench');
    for (const [id, weapon] of Object.entries(WEAPONS)) {
      const unlocked = state.unlocked.includes(id), equipped = state.weapon === id;
      cards.push(manualCard({
        title: weapon.name, tag: equipped ? '使用中' : unlocked ? '已解锁' : '未解锁', owned: equipped,
        body: weapon.text,
        stats: [`伤害 ${weaponDamage(state, id)}`, `射程 ${weaponRange(state, id)}`, `间隔 ${weapon.interval}s`],
        actionText: equipped ? '已装备' : unlocked ? '装备' : '解锁并装备 · ⚙10',
        disabled: equipped || (!unlocked && (!bench || state.scrap < 10 || state.phase !== 'day')),
        disabledText: !unlocked && !bench ? '需要便携工作台：进入房车安装后才能解锁武器。' : !unlocked && state.phase !== 'day' ? '只能在白天解锁武器。' : !unlocked && state.scrap < 10 ? '零件不足（需要 ⚙10）· 远征可获得零件。' : '',
        action: () => {
          const ok = unlocked ? equipWeapon(state, id) : unlockWeapon(state, id);
          if (!ok) { toast('无法更换武器。'); return; }
          audio.effect('build'); toast(`${unlocked ? '装备' : '解锁'}武器 · ${weapon.name}`); renderManual(); syncUI();
        },
      }));
    }
    if (state.phase !== 'day') cards.push(Object.assign(document.createElement('p'), { className: 'empty', textContent: '夜间无法在工坊解锁与更换武器，天亮后再来。' }));
  } else if (manualTab === 'expedition') {
    if (state.phase !== 'day') {
      cards.push(Object.assign(document.createElement('p'), { className: 'empty', textContent: '远征只能在白天出发。夜晚必须留在营地防守。' }));
    } else {
      for (const trip of EXPEDITIONS) {
        ...
        const reason = done ? '今天已经出发过一次，明天再来。' : noTime ? '剩余白天时间不足，无法完成这次行动。' : hurt ? '生命不足，先恢复再出发。' : '';
        cards.push(manualCard({
          title: trip.name, tag: trip.tag,
          body: trip.text.replace(`耗时 ${trip.time} 秒`, `白天时间 −${trip.time} 秒`),
          actionText: done ? '今日已完成' : '出发',
          disabled: !!reason, disabledText: reason,
          action: () => {
            if (!expedition(state, trip.id)) { toast('现在无法出发。'); return; }
            audio.effect('collect'); toast(`远征归来 · ${trip.name}`); renderManual(); syncUI();
          },
        }));
      }
    }
    ...
    cards.push(manualCard({
      title: `第 ${state.day} 夜 · ${wave.name}`, tag: when,
      body: wave.lesson,
      stats: [`木材 ▰${state.wood}`, `零件 ⚙${state.scrap}`, `营地 ${Math.ceil(state.health)}%`, `生命 ${Math.ceil(state.playerHp)}/${maxHp(state)}`, `瞭望塔 ${buildings.filter(b => b.type === 'tower').length}`, `医疗包 ✚${state.medkits}`],
    }));
    if (wave.fog) cards.push(manualCard({ title: `浓雾 · 能见度 ${Math.round((1 - wave.fog) * 100)}%`, tag: '环境', body: '灯光与信号弹的照亮范围缩小；潜行者在灯外几乎不可见，腐吐者借雾远程腐蚀建筑。' }));
    cards.push(manualCard({ title: '应对建议', tag: '战术', body: wave.advice }));
    wave.groups.forEach(([type, count, lane, , intent]) => {
      const def = ENEMY_TYPES[type];
      cards.push(manualCard({
        title: `${def.name} ×${count}`, tag: `${LANES[lane]} · ${intent || '来袭'}`,
        body: `${def.note} · 生命 ${def.hp} · 速度 ${def.speed}${def.armor ? ` · 护甲 ${def.armor}` : ''}`,
      }));
    });
    if (hasFurniture(state, 'radio')) {
      for (const card of rv.timelineCards(state.day)) cards.push(manualCard({ title: card.title, tag: card.tag, stats: card.rows }));
    } else {
      cards.push(manualCard({
        title: '短波电台', tag: '房车 · 未安装',
...
function openManual() {
  if (inside) { toast('先离开房车，再查看营地手册。'); return; }
  if (state.over || rvTransition || $('#manual-dialog').open) return;
  renderManual(); actor.send({ type: 'OPEN_MANUAL' }); $('#manual-dialog').showModal();
}
function closeManual() { if ($('#manual-dialog').open) $('#manual-dialog').close(); }
document.querySelectorAll('[data-manual-tab]').forEach(tab => tab.addEventListener('click', () => { manualTab = tab.dataset.manualTab; renderManual(); }));
$('#manual-close').addEventListener('click', closeManual);
$('#manual-dialog').addEventListener('close', () => actor.send({ type: 'CLOSE_MANUAL' }));
function cycleWeapon() {
  if (state.unlocked.length < 2) { toast('在营地手册（Tab）的工坊解锁更多武器。'); return; }
  const index = state.unlocked.indexOf(state.weapon);
  equipWeapon(state, state.unlocked[(index + 1) % state.unlocked.length]);
  toast(`切换武器 · ${WEAPONS[state.weapon].name}`); syncUI();
}
function tryDash() {
  if (inside || rvTransition || state.paused || state.over) return;
  if (state.stamina < DASH.cost) { toast('体力不足，稍作休息。'); return; }
  state.stamina -= DASH.cost; state.staminaDelay = DASH.delay;
  playerInvuln = Math.max(playerInvuln, DASH.invulnerable);
  const angle = player.rotation.y;
  dash = { time: DASH.duration, dx: Math.sin(angle), dz: Math.cos(angle) };
  audio.effect('dash'); burst(player.position.x, .5, player.position.z, 5);
}
function tryFlare() {
  if (inside || rvTransition || state.paused || state.over) return;
  if (!useFlare(state)) { toast(`信号弹冷却中 · ${Math.ceil(state.flareCooldown)} 秒`); return; }
  const angle = player.rotation.y;
  const x = player.position.x + Math.sin(angle) * 1.2, z = player.position.z + Math.cos(angle) * 1.2;
  const flare = flareVisual();
  flare.active = true; flare.life = FLARE.duration; flare.x = x; flare.z = z;
  flare.group.position.set(x, 0, z); flare.group.visible = true; flare.light.intensity = 36;
  burst(x, .8, z, 14); audio.effect('flare');
  toast('信号弹升空 · 范围内的敌人被压制减速。');
}
function tryHeal() {
  if (inside || rvTransition || state.paused || state.over) return;
  if (heal(state)) { audio.effect('heal'); burst(player.position.x, .8, player.position.z, 10); toast(`医疗包 · 生命 +${MEDKIT_HEAL}`); syncUI(); }
...
function hurtPlayer(amount) {
  ...
  if (downed) {
    playerDown(state);
    nightRun.downs++;
    playerInvuln = 2.5; dash = null; movementTarget = null;
...
addEventListener('keydown', event => {
  ...
  if ($('#manual-dialog').open) {
    if (key === 'tab') { event.preventDefault(); closeManual(); }
    return;
  }
  if (event.target instanceof HTMLInputElement || $('#help-dialog').open || $('#end-dialog').open || $('#perk-dialog').open) return;
  if (['arrowup', 'arrowdown', 'arrowleft', 'arrowright'].includes(key) || key === 'tab' || (key === ' ' && !(event.target instanceof HTMLButtonElement))) event.preventDefault();
  ...
  if (key === 'escape') { if (selectedBuilding) selectBuilding(null); else if (inside) enterOrExit(); else selectBuild(null); }
  if (key === 'r' && ghost) { ghostAngle += Math.PI / 2; ghost.rotation.y = ghostAngle; }
  if (key === 'e') collect(); if (key === 'n') transition(); if (key === 'h') togglePhoto(); if (key === 'p') togglePause();
  if (key === 'shift') tryDash(); if (key === 'q') tryFlare(); if (key === 'f') tryHeal(); if (key === 'x') cycleWeapon();
});
...
function movePlayer(dt) {
  ...
  if (dashing) { dx = dash.dx; dz = dash.dz; length = 1; }
  const moving = length > .01;
  if (moving) {
  ...
  if (dashing) { dash.time -= dt; if (dash.time <= 0) dash = null; }
...
const LANE_START = [[-2, -29], [-28, -.5], [18, 9]];
function spawn(entry) {
  const record = enemyPools[entry.type].find(e => !e.alive);
  if (!record) return false;
  const def = ENEMY_TYPES[entry.type], [x, z] = LANE_START[entry.lane] || LANE_START[0];
  record.mesh.position.set(x + (random() - .5) * 3, 0, z + (random() - .5) * 3);
  record.mesh.rotation.set(0, 0, 0);
  record.mesh.userData.body.rotation.x = 0;
  record.hp = record.maxHp = def.hp; record.attack = 0; record.slow = 0; record.slowFactor = 1;
  record.flash = 0; record.dying = 0; record.kx = 0; record.kz = 0;
  setEnemyMaterial(record, enemyMaterials.skin);
  record.alive = true; record.mesh.visible = true;
...
function tracer(from, target, color = '#ffe6a1') {
  ...
  v.set(end.x - from.x, .85 - from.y, end.z - from.z);
  shot.mesh.position.set(from.x + v.x / 2, from.y + v.y / 2, from.z + v.z / 2);
  shot.mesh.scale.set(.035, v.length(), .035);
  shot.mesh.quaternion.setFromUnitVectors(T.Object3D.DEFAULT_UP, v.normalize());
  shot.mesh.material.color.set(color);
  shot.mesh.visible = true; shot.life = .075;
}
function spawnMuzzle(x, y, z, dx, dz) {
  const flash = muzzles.find(f => f.life <= 0); if (!flash) return;
  const m = flash.mesh;
  m.position.set(x, y, z);
  m.quaternion.setFromUnitVectors(T.Object3D.DEFAULT_UP, v.set(dx, .05, dz).normalize());
  m.scale.set(.15 + random() * .06, .22 + random() * .12, .15 + random() * .06);
  m.visible = true; flash.life = .055;
}
function damageBuilding(b, amount) {
  const view = viewOf(b);
  b.hp -= amount; view.hit = .16;
  if (b.hp > 0) { audio.effect('damage'); trauma.add(.05); return false; }
  audio.effect('destroy'); trauma.add(.5);
  burst(b.x, 1, b.z, 16);
  detachBuildingView(b);
  buildings.splice(buildings.indexOf(b), 1);
...
function enemyVisible(e) {
  ...
  const lanterns = buildings.filter(b => b.type === 'lantern').map(b => ({ x: b.x, z: b.z, radius: lanternRadius(b) }));
...
function fireSpit(enemy, goal, def) {
  const spit = spits.find(s => s.life <= 0); if (!spit) return;
  const p = enemy.mesh.position;
  spit.from.set(p.x, .95, p.z);
  spit.to.set(goal.kind === 'building' ? goal.building.x : player.position.x, .8, goal.kind === 'building' ? goal.building.z : player.position.z);
  spit.total = Math.max(.28, spit.from.distanceTo(spit.to) / 9);
  spit.life = spit.total; spit.kind = goal.kind; spit.ref = goal.building || null; spit.damage = def.damage;
  spit.mesh.position.copy(spit.from); spit.mesh.visible = true; spitCount++;
}
function applySpitHit(spit) {
  burst(spit.mesh.position.x, spit.mesh.position.y, spit.mesh.position.z, 4);
  if (spit.kind === 'building') { if (buildings.includes(spit.ref)) damageBuilding(spit.ref, spit.damage + 3); return; }
  if (Math.hypot(player.position.x - spit.to.x, player.position.z - spit.to.z) < 1.3) hurtPlayer(spit.damage);
}
function damageEnemy(enemy, damage, fromX, fromZ) {
  const def = ENEMY_TYPES[enemy.type];
  enemy.hp -= damage;
  const p = enemy.mesh.position;
  burst(p.x, .9, p.z, damage >= 4 ? 6 : 3);
  if (enemy.hp > 0) {
    enemy.flash = .12; setEnemyMaterial(enemy, enemyFlashMaterial); hitFlashes++;
    ...
    if (Number.isFinite(fromX) && Number.isFinite(fromZ) && enemy.type !== 'alpha') {
      ...
      const power = Math.min(3.2, 1.2 + damage * .45) * (enemy.type === 'brute' ? .35 : 1);
      enemy.kx += dx / d * power; enemy.kz += dz / d * power; knockbacks++;
    }
    return;
  }
  enemy.alive = false; enemy.dying = .5; enemy.flash = 0;
  state.kills++; state.wood += def.bounty; deaths++; audio.effect('kill');
  hitStop.trigger(hitStopFor(def.hp)); trauma.add(killTraumaFor(def.hp));
...
function updateEnemyEffects(dt) {
  for (const e of enemies) {
    if (e.flash > 0) {
      ...
      e.mesh.userData.body.rotation.x = -e.flash / .12 * .3;
      if (!e.flash) { setEnemyMaterial(e, enemyMaterials.skin); e.mesh.userData.body.rotation.x = 0; }
    }
    if (e.dying > 0) {
      e.dying -= dt;
      const t = 1 - Math.max(0, e.dying) / .5;
      e.mesh.userData.body.rotation.x = 0;
      e.mesh.rotation.x = -1.35 * t;
      e.mesh.position.y = -.3 * t;
      if (e.dying <= 0) { e.mesh.visible = false; e.mesh.rotation.x = 0; e.mesh.position.y = 0; setEnemyMaterial(e, enemyMaterials.skin); }
    }
  }
}
...
function inRange(x, z, range, count) {
  const list = [];
  for (const e of enemies) if (e.alive && enemyVisible(e)) {
...
function towerFire(b, target) {
  ...
  shotFrom.set(b.x, 4.3, b.z); tracer(shotFrom, target);
  spawnMuzzle(b.x, 4.36, b.z, target.mesh.position.x - b.x, target.mesh.position.z - b.z);
  muzzleLight.position.set(b.x, 4.4, b.z); muzzleLight.color.set('#ffe6a1'); muzzleLight.intensity = 20;
  muzzleFlashes++;
  damageEnemy(target, applyArmor(attackDamage(state, stats.damage, true), def.armor, .25), b.x, b.z);
  audio.effect('tower');
}
function playerAttack(dt) {
  ...
  const targets = inRange(player.position.x, player.position.z, weaponRange(state, state.weapon), weapon.targets);
  ...
  for (const target of targets) {
    ...
    damageEnemy(target, applyArmor(base, def.armor, weapon.armorPen), player.position.x, player.position.z);
  }
  shotTimer = weapon.interval; muzzleFlashes++; recoils++;
  player.userData.recoil = 1;
  spawnMuzzle(player.position.x, 1.05, player.position.z, first.x - player.position.x, first.z - player.position.z);
  muzzleLight.position.set(player.position.x, 1.1, player.position.z); muzzleLight.color.set(weapon.color); muzzleLight.intensity = 26;
  audio.effect('shoot');
}
function combat(dt) {
  ...
  while (spawnQueue.length && spawnQueue[0].at <= nightClock) {
    ...
    spawnQueue.shift(); remaining--;
  }
  playerAttack(dt);
  for (const b of buildings) if (b.type === 'tower' && viewOf(b).growth >= 1) {
    const view = viewOf(b);
    view.cooldown -= dt;
    if (view.cooldown > 0) continue;
    const enemy = inRange(b.x, b.z, towerStats(b).range, 1)[0];
    if (enemy) { towerFire(b, enemy); view.cooldown = towerStats(b).cooldown; }
  }
  for (const e of enemies) if (e.alive) {
    const def = ENEMY_TYPES[e.type], p = e.mesh.position;
    e.mesh.visible = enemyVisible(e);
    let tx = -1, tz = -1.6;
    // Siege enemies ignore the lanes and go straight for a tower; the rest follow clear paths.
    if (def.siege) { const tower = siegeGoal(p.x, p.z, buildings, def.siege); if (tower) { tx = tower.x; tz = tower.z; } }
    else if (p.z < -12) { tx = -1; tz = -10; } else if (p.x < -13) { tx = -10; tz = 4; }
    const dx = tx - p.x, dz = tz - p.z, length = Math.hypot(dx, dz);
    let speed = def.speed;
    for (const b of buildings) if (b.type === 'lantern' && Math.hypot(b.x - p.x, b.z - p.z) < lanternRadius(b)) speed *= lanternSlow(b);
    for (const flare of flarePool) if (flare.active && Math.hypot(flare.x - p.x, flare.z - p.z) < FLARE.radius) {
      e.slow = .25; e.slowFactor = e.type === 'alpha' ? FLARE.bossSlow : FLARE.slow;
    }
    ...
    const ranged = def.ranged && e.attack <= 0 ? rangedGoal(p.x, p.z, player.position.x, player.position.z, buildings, def.range) : null;
    if (ranged) {
      e.attack = def.attackRate;
      fireSpit(e, ranged, def);
      e.mesh.rotation.y = Math.atan2((ranged.building ? ranged.building.x : player.position.x) - p.x, (ranged.building ? ranged.building.z : player.position.z) - p.z);
    } else if (barrier) {
      if (e.attack <= 0) { e.attack = def.attackRate; burst(p.x, .7, p.z, 2); damageBuilding(barrier, 9 * def.wallDamage); }
    } else if (playerNear && e.attack <= 0) {
      e.attack = def.attackRate; hurtPlayer(def.damage);
    } else if (rvCollision(p.x, p.z, .2)) {
      if (e.attack <= 0) { damageCamp(state, def.damage); audio.effect('damage'); e.attack = def.attackRate * 1.2; trauma.add(.45); flashHurt(); syncUI(); actor.send({ type: 'CAMP_DESTROYED' }); }
    } else if (length > .01) { p.x += dx / length * dt * speed; p.z += dz / length * dt * speed; }
    if (e.kx || e.kz) {
      const nx = p.x + e.kx * dt, nz = p.z + e.kz * dt;
      if (!rvCollision(nx, nz, .1)) { p.x = nx; p.z = nz; }
      const damp = Math.exp(-dt * 11); e.kx *= damp; e.kz *= damp;
      if (Math.abs(e.kx) < .02) e.kx = 0; if (Math.abs(e.kz) < .02) e.kz = 0;
    }
    if (!ranged) e.mesh.rotation.y = Math.atan2(dx, dz);
    e.mesh.userData.body.position.y = Math.abs(Math.sin(time * 7 + e.seed)) * .06;
    e.mesh.userData.legs.forEach((leg, i) => leg.rotation.x = Math.sin(time * 7 + e.seed + i * Math.PI) * .4);
  }
  if (!remaining && !spawnQueue.length && !enemies.some(e => e.alive) && state.elapsed > 5) transition();
}
function updateBossBar() {
  const boss = enemies.find(e => e.alive && e.type === 'alpha'), bar = $('#boss-bar');
  if (!boss) { if (!bar.hidden) bar.hidden = true; return; }
  bar.hidden = false;
...
function endGame() {
  ...
  keys.clear(); movementTarget = null; selectBuild(null); selectBuilding(null);
...
function ambience(dt) {
  daylight = T.MathUtils.damp(daylight, homeMode ? .18 : state.phase === 'day' ? 1 : 0, .55, dt);
  ...
  moon.intensity = (1 - daylight) * .5;
  const fog = currentFog();
  moon.intensity = (1 - daylight) * .5 * (1 - fog * .55);
  world.doorLight.intensity = (1 - daylight) * 7;
  updateRvGlow(world, state.rv, daylight, interior ? interior.lampOn : true, ambientTime);
  playerLight.intensity = (1 - daylight) * 9 * (1 - fog * .45);
  playerLight.distance = 8 - fog * 3;
  playerLight.position.set(player.position.x, 1.6, player.position.z + .3);
  renderer.toneMappingExposure = .9 + daylight * .14 - (1 - daylight) * fog * .07;
  ...
  for (const s of spits) if (s.life > 0) {
    s.life -= dt;
    const k = 1 - Math.max(0, s.life) / s.total;
    s.mesh.position.lerpVectors(s.from, s.to, k);
    s.mesh.position.y = s.from.y + (s.to.y - s.from.y) * k + Math.sin(k * Math.PI) * .7;
    if (s.life <= 0) { s.mesh.visible = false; applySpitHit(s); }
  }
  for (const f of muzzles) if (f.life > 0) { f.life -= dt; f.mesh.visible = f.life > 0; f.mesh.scale.multiplyScalar(Math.exp(-dt * 9)); }
  // Recoil kicks the rifle back, then the arm settles.
  player.userData.recoil = Math.max(0, (player.userData.recoil || 0) - dt * 6.5);
  if (player.userData.gun) player.userData.gun.position.z = .34 - player.userData.recoil * .07;
  player.userData.body.rotation.x = player.userData.recoil * .1;
  for (const flare of flarePool) if (flare.active) {
    flare.life -= dt;
    flare.light.intensity = flare.life > 0 ? 34 + Math.sin(t * 22) * 9 * motion : 0;
    flare.orb.scale.setScalar(1 + Math.sin(t * 18) * .16 * motion);
    if (flare.life <= 0) { flare.active = false; flare.group.visible = false; }
  }
  ...
  for (const b of buildings) {
    const view = viewOf(b);
    if (view.growth < 1) { view.growth = Math.min(1, view.growth + dt * 1.5); view.mesh.scale.setScalar(easeOutBack(view.growth)); }
    if (view.hit > 0) { view.hit = Math.max(0, view.hit - dt); view.mesh.rotation.z = Math.sin(time * 46) * .03 * (view.hit / .16); if (!view.hit) view.mesh.rotation.z = 0; }
    if (view.light) { view.light.intensity = (4 + (1 - daylight) * 20) * (1 - fog * .5); view.light.distance = 9 - fog * 3.5; }
  }
...
let uiTimer = 0, titleAngle = 0;
function updateTitleCamera(dt) {
  titleAngle += dt * .06;
  const aspect = innerWidth / innerHeight, size = innerWidth < 650 ? 11 : 9;
  camera.left = -size * aspect; camera.right = size * aspect; camera.top = size; camera.bottom = -size;
  camera.updateProjectionMatrix();
  const radius = 14 + Math.sin(titleAngle * .3) * 1.5;
  const px = pointerInside ? pointer.x : 0;
  cameraCenter.set(-1, 0, 2);
  camera.position.set(-1 + Math.cos(titleAngle) * radius + px * 1.4, 5.6 + Math.sin(titleAngle * .9) * .7, 2.2 + Math.sin(titleAngle) * radius * .8);
  camera.lookAt(-1 + px * .8, 1.5, 1.4);
...
renderer.setAnimationLoop(now => {
  const realDt = Math.max(0, Math.min((now - last) / 1000, .05)); last = now;
  ...
  if (!state.paused && !state.over && !document.hidden) {
    ...
    if (inside) movePlayer(dt);
      ...
      state.elapsed += dt; tickSurvival(state, dt); ambience(dt); movePlayer(dt); combat(dt); updateEnemyEffects(dt);
  ...
  if (shakeEnabled) { camera.position.x += shake.x; camera.position.z += shake.z; camera.rotation.z += shake.roll; }
  camera.updateMatrixWorld();
  sun.position.set(cameraCenter.x - 26, 35, cameraCenter.z - 22);
  sun.target.position.set(cameraCenter.x, 0, cameraCenter.z);
  moon.position.set(cameraCenter.x + 24, 30, cameraCenter.z + 20);
...
const actor = startCampaign({
  ...
  onNightStart: () => {
    ...
    audio.effect('night');
    selectBuild(null); selectBuilding(null);
  },
  onDawn: () => {
    world.logs.forEach(l => l.remaining = 4);
    shotTimer = 0;
    toast('天亮了。营地补给已送达，按 Tab 查看工坊与远征。');
    audio.update(state, inside); audio.effect('dawn');
    selectBuild(null); selectBuilding(null);
    openPerks();
  },
  onVictory: () => endGame(),
...
const home = createHome({
  ...
  onContinue: () => { const loaded = readSave(); if (loaded.ok) continueCampaign(loaded.save); },
  onSpark: () => { burst(.1, 1, 5.6, 9); audio.effect('collect'); },
  onPointer: (clientX, clientY) => { pointer.set(clientX / innerWidth * 2 - 1, 1 - clientY / innerHeight * 2); pointerInside = true; },
  onLeave: () => { pointerInside = false; },
});
function clearSession() {
  selectBuild(null); selectBuilding(null);
  keys.clear(); movementTarget = null; shotTimer = 0; nightClock = 0; spawnQueue = []; remaining = 0;
}
function startNewCampaign() {
  resetCampaign(state);
  state.logs.forEach(log => { log.remaining = 4; });
  syncBuildingScene();
  clearSession();
  enterGame();
  const saved = writeSave(state);
...
function enterHome() {
  ...
  home.show({ save: loaded.ok ? saveSummary(loaded.save) : null, broken: !loaded.ok && loaded.reason !== 'missing' && loaded.reason !== 'storage' });
  pointerInside = false;
  syncUI();
}
...
window.__pinefall = {
  ...
  get machine() {
    ...
    return { phase: phaseValue(snap), overlay: overlayValue(snap), interior: isInterior(snap), paused: snap.context.paused };
  },
  get home() { return { visible: home.visible, mode: homeMode, enabled: homeEnabled }; },
  startNew: () => startNewCampaign(),
  continueSave: () => { const loaded = readSave(); if (loaded.ok) continueCampaign(loaded.save); return loaded.ok; },
  get loading() { return { progress: loadingProgress, ready: loading.classList.contains('done') }; },
  get state() { return { ...state, perks: [...state.perks], unlocked: [...state.unlocked] }; },
  get stats() {
    return {
      inside, transitioning: !!rvTransition, lampOn: interior ? interior.lampOn : null, manualOpen: $('#manual-dialog').open,
      viewZoom: inside ? interior.camera.zoom : camera.zoom, calls: renderer.info.render.calls, triangles: renderer.info.render.triangles,
      enemies: enemies.filter(e => e.alive).length, remaining, ambientTime, windStrength: world.wind.strength.value,
      wireZ: world.lightWire.geometry.attributes.position.getZ(20), fireIntensity: world.fireLight.intensity,
      buildings: buildings.length, buildingLevels: buildings.map(b => `${b.type}:${b.level}:${Math.ceil(b.hp)}`),
      selectedBuilding: selectedBuilding ? selectedBuilding.type : null,
      camera: cameraCenter.toArray(), player: player.position.toArray(),
      playerHp: state.playerHp, maxHp: maxHp(state), stamina: state.stamina, medkits: state.medkits, scrap: state.scrap,
      weapon: state.weapon, enemyTypes: enemies.filter(e => e.alive).reduce((counts, e) => (counts[e.type] = (counts[e.type] || 0) + 1, counts), {}),
      flareActive: flarePool.some(f => f.active), flareCooldown: state.flareCooldown, dashing: !!dash, invulnerable: playerInvuln > 0,
      shakeEnabled, trauma: trauma.value, shakeFrames: trauma.frames, muzzleFlashes, flashes: hitFlashes,
      dying: enemies.filter(e => e.dying > 0).length, deaths, hitStops: hitStop.count, knockbacks, recoils,
      fog: currentFog(), hidden: enemies.filter(e => e.alive && !enemyVisible(e)).length,
      spits: spitCount, sieging: enemies.filter(e => e.alive && ENEMY_TYPES[e.type].siege && siegeGoal(e.mesh.position.x, e.mesh.position.z, buildings, ENEMY_TYPES[e.type].siege)).length,
      rating: lastRating ? { ...lastRating } : null,
      rv: [...state.rv], rvSlots: rvSlots(state), weaponMod: state.weaponMod, maxMedkits: maxMedkits(state),
      playerDamage: weaponDamage(state, state.weapon), playerRange: weaponRange(state, state.weapon),
