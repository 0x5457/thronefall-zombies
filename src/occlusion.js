...
export function addOcclusionSilhouette(character) {
  const silhouette = new T.MeshBasicMaterial({
    color: '#a8eee5', transparent: true, opacity: .48,
    depthFunc: T.GreaterDepth, depthWrite: false, toneMapped: false, fog: false,
    stencilWrite: true, stencilRef: 1, stencilFunc: T.NotEqualStencilFunc,
    stencilZPass: T.ReplaceStencilOp,
  });
  const parts = [];
  character.traverse(part => { if (part.isMesh) parts.push(part); });
