...
export function createRv({ state, getInterior, audio, toast, syncUI }) {
  ...
  function render() {
    if (!list) return;
    const functions = Object.entries(RV_FURNITURE).filter(
      ([, furniture]) => furniture.kind === 'function',
    );
    const decor = Object.entries(RV_FURNITURE).filter(
      ([, furniture]) => furniture.kind === 'decor',
    );
    const children = [
      el(
        'p',
