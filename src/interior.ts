...
export function makeInterior() {
  ...
  const markers: Record<string, T.Mesh<T.BufferGeometry, T.MeshBasicMaterial>> = {};
  ...
  function applyFurniture(owned: readonly string[] = []): void {
  ...
  function applySlots(functionSlots: number = 1): void {
    let index = 0;
  ...
  return {
    ...
    markers,
    slots: RV_SLOTS,
    setFurniture(owned: readonly string[]) {
      applyFurniture(owned);
    },
    setSlots(functionSlots: number) {
...
export type Interior = ReturnType<typeof makeInterior>;

// Slot anchors in interior space: functions on the cut edge, decor on counters, bed and wall.
export const RV_SLOTS: Record<string, RvSlot> = {
...
function slotRing(x: number, z: number): T.Mesh<T.BufferGeometry, T.MeshBasicMaterial> {
...
function decorPin(x: number, y: number, z: number): T.Mesh<T.BufferGeometry, T.MeshBasicMaterial> {
...
function workbenchModel(): T.Group {
  const g = new T.Group();
...
export function interiorBlocked(x: number, z: number): boolean {
