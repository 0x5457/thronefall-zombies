...
import type { GameState, PerkId } from './rules.js';
...
export type CampaignEvent =
  | { type: 'START_NIGHT' }
  | { type: 'CLEARED' }
  | { type: 'CHOOSE_PERK'; perkId: PerkId }
  | { type: 'ENTER_RV' }
  | { type: 'EXIT_RV' }
  | { type: 'PAUSE' }
  | { type: 'RESUME' }
...
export interface CampaignOptions {
  state?: GameState;
  canClear?: () => boolean;
  canEnterRV?: (s: GameState) => boolean;
  onNightStart?: (s: GameState) => void;
  onDawn?: (s: GameState) => void;
  onVictory?: (s: GameState) => void;
  onDefeat?: (s: GameState) => void;
  onPerkChosen?: (s: GameState, id: PerkId) => void;
}

export function perkAvailable(s: GameState, id: string): boolean {
  return (
    !s.over &&
    s.phase === 'day' &&
    s.perkPending &&
...
export function createCampaignMachine(options: CampaignOptions = {}) {
  const {
    state = newGame(),
    canClear = () => true,
    canEnterRV = () => true,
  ...
  return setup({
    ...
    guards: {
      ...
      perkAvailable: ({ context, event }) =>
        perkAvailable(context, (event as Extract<CampaignEvent, { type: 'CHOOSE_PERK' }>).perkId),
    ...
    actions: {
      ...
      takePerk: ({ context, event }) => {
        const { perkId } = event as Extract<CampaignEvent, { type: 'CHOOSE_PERK' }>;
        choosePerk(context, perkId);
        context.paused = false;
        onPerkChosen(context, perkId);
      },
...
export function startCampaign(options: CampaignOptions = {}) {
...
export const phaseValue = (snapshot: CampaignSnapshot): string | null => {
  const value = snapshot.value?.phase;
  if (!value) return null;
  return typeof value === 'object' ? Object.keys(value)[0] : value;
};

export const overlayValue = (snapshot: CampaignSnapshot): string =>
  snapshot.value?.overlay ?? 'none';

export const isInterior = (snapshot: CampaignSnapshot): boolean =>
  (snapshot.value?.phase as { day?: string } | undefined)?.day === 'interior';

export const effectivelyPaused = (snapshot: CampaignSnapshot): boolean =>
  overlayValue(snapshot) !== 'none' || phaseValue(snapshot) === 'dawn';

export function assertPhaseSync(snapshot: CampaignSnapshot): void {
