...
export const applyArmor = (damage, armor = 0, penetration = 0) =>
  damage *
  (1 - Math.min(ARMOR_CAP, Math.max(0, armor) * (1 - Math.max(0, Math.min(1, penetration)))));
export function canBuild(s, type) {
  return Object.hasOwn(COSTS, type) && active(s) && s.phase === 'day' && s.wood >= COSTS[type];
}
export function buy(s, type) {
...
export function overlaps(x, z, objects, radius = 1) {
...
export const attackDamage = (s, base, tower = false) =>
  base *
  (s.perks.includes(tower ? 'engineer' : 'marksman')
    ? PERKS[tower ? 'engineer' : 'marksman'].damageMultiplier
    : 1);
...
export function equipWeapon(s, id) {
  if (s.over || !Object.hasOwn(WEAPONS, id) || !s.unlocked.includes(id)) return false;
  s.weapon = id;
  return true;
}
export const hasFurniture = (s, id) => s.rv.includes(id);
export const furnitureCount = (s, kind) =>
...
export const weaponModActive = (s) =>
...
export const weaponDamage = (s, id) =>
  WEAPONS[id].damage + (weaponModActive(s) ? WEAPON_MODS[s.weaponMod].damage : 0);
export const weaponRange = (s, id) =>
  WEAPONS[id].range + (weaponModActive(s) ? WEAPON_MODS[s.weaponMod].range : 0);
