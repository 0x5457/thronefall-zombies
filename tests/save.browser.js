...
try {
  ...
  const game = () => page.evaluate(() => ({ ...window.__pinefall.state, ...window.__pinefall.stats }));
  ...
  assert.equal(await page.evaluate(() => JSON.parse(localStorage.getItem('pinefall.campaign.current')).state.day), 1);
  await build(-2, -8); await build(8, 2);
  ...
  const checkpoint = await page.evaluate(() => JSON.parse(localStorage.getItem('pinefall.campaign.current')));
  ...
  assert.ok(checkpoint.state.logs.every(l => typeof l.id === 'string'), 'logs serialize with stable ids');
  ...
  await page.waitForSelector('#loading.done', { timeout: 60000 });
  await page.waitForSelector('#home.shown');
  assert.match(await page.locator('#home-save-info').textContent(), /第 1 天/, 'backup recovered');
  ...
  await page.evaluate(() => {
    ...
    future.version = 99;
    for (const key of ['pinefall.campaign.current', 'pinefall.campaign.backup']) localStorage.setItem(key, JSON.stringify(future));
  });
  await page.goto(`${BASE}?home=1`);
  await page.waitForSelector('#loading.done', { timeout: 60000 });
  await page.waitForSelector('#home.shown');
  assert.equal(await page.locator('#home-continue').isDisabled(), true, 'newer versions are never loaded');
  assert.match(await page.locator('#home-save-info').textContent(), /损坏|无法/, 'the refusal is explained');
  const kept = await page.evaluate(() => JSON.parse(localStorage.getItem('pinefall.campaign.current')).version);
  assert.equal(kept, 99, 'refusing a save never rewrites it');
  await page.screenshot({ path: 'artifacts/save-home-refused.png' });

  assert.deepEqual(errors, [], 'no page errors');
  console.log('PASS: real dawn checkpoint saves, title continue resumes it, corrupt falls back, future version refused');
