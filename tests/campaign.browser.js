...
try {
  ...
  async function build(x, z) {
    ...
    assert.ok(!(await page.locator('#world-label').getAttribute('class')).includes('invalid'), `valid tower at ${x},${z}`);
  ...
  await page.waitForFunction(() => window.__pinefall.state.perkPending || window.__pinefall.state.over, null, { timeout: 240000 });
  const dawn = await game(); console.log('first dawn', dawn);
  assert.equal(dawn.over, false, 'real defense survived');
  assert.equal(dawn.kills, 10); assert.equal(dawn.day, 2);
  assert.equal(await page.locator('#perk-options button').count(), 4);
  assert.equal(dawn.paused, true);
  await page.keyboard.press('Escape'); await page.keyboard.press('n'); await page.keyboard.press('p');
  ...
  assert.ok(await page.locator('#perk-dialog').evaluate(d => d.getBoundingClientRect().right <= innerWidth));
  await page.locator('[data-perk="marksman"]').focus(); await page.keyboard.press('Enter');
  ...
  await page.keyboard.press('n'); assert.equal((await game()).phase, 'night');
  assert.equal((await game()).remaining, 16);
  assert.deepEqual(errors, []);
  console.log('PASS: real first-night defense → modal/input lock → keyboard reward → second night; no state injection');
