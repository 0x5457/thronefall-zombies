...
import {
  newGame, advance, choosePerk, installFurniture, uninstallFurniture, canInstallFurniture, furnitureReason,
  rvSlots, hasFurniture, maxMedkits, setWeaponMod, weaponDamage, weaponRange, spawnTimeline, unlockWeapon,
  RV, RV_FURNITURE, WEAPON_MODS, heal, MEDKIT_HEAL,
} from '../src/rules.js';
...
test('function slots unlock across days while decoration is available from day one', () => {
  ...
  assert.equal(s.wood, 55); assert.equal(s.scrap, 4);
  assert.equal(canInstallFurniture(s, 'radio'), false, 'only one function slot before the first night');
  ...
  advance(s); advance(s, true); choosePerk(s, 'marksman');
  assert.equal(rvSlots(s), 2);
  s.wood = 100; s.scrap = 20;
  ...
  advance(s); advance(s, true); choosePerk(s, 'engineer');
  s.wood = 100; s.scrap = 20;
...
test('installing is gated by daylight and resources; dismantling refunds 60%', () => {
  ...
  s.wood = 80;
  assert.equal(installFurniture(s, 'radio'), true);
  assert.equal(s.wood, 70); assert.equal(s.scrap, 2);
  const night = newGame(); night.phase = 'night';
  assert.match(furnitureReason(night, 'radio'), /白天/);
  const day1 = newGame(); installFurniture(day1, 'radio');
...
test('the workbench gates weapon unlocks and grants one nightly mod that resets at dawn', () => {
  ...
  advance(s, true); choosePerk(s, 'marksman');
...
test('radio timeline collapses consecutive spawns into readable rows', () => {
  ...
  for (const row of wide) { assert.ok(row.count >= 1); assert.ok(row.end >= row.at); }
