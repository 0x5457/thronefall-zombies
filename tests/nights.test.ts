...
test('siege enemies pick the nearest tower inside their range and nothing otherwise', () => {
  ...
  assert.equal(
    siegeGoal(0, 0, [{ type: 'fence', x: 0.5, z: 0 } as Building], 15),
    null,
    'fences are not siege goals',
  );
});

test('ranged enemies prefer buildings, then the player, then keep walking', () => {
  const buildings = [{ type: 'lantern', x: 4, z: 0 } as Building];
  assert.deepEqual(rangedGoal(0, 0, 50, 50, buildings, 6.5), {
    kind: 'building',
  ...
  assert.equal(
    rangedGoal(0, 0, 50, 50, [{ type: 'tower', x: 40, z: 0 } as Building], 6.5),
    null,
    'distant buildings are ignored',
  );
});
...
test('stealth reveal comes from lamps, flares and close range, and fog is required to hide', () => {
  ...
  assert.equal(revealed(3, 0, 99, 99, lamps, [], 0.4), true, 'inside lamp radius');
  assert.equal(revealed(20, 0, 99, 99, [], flares, 0.4), true, 'inside an active flare');
  assert.equal(revealed(20, 0, 99, 99, [], inactive, 0.4), false, 'expired flares reveal nothing');
  assert.equal(revealed(12, 0, 10, 0, [], [], 0.4), true, 'players reveal what is on top of them');
  assert.equal(revealed(12, 0, 30, 0, [], [], 0.4), false, 'otherwise fog hides');
});

test('new enemy stats stay bounded and armor applies as usual', () => {
  for (const type of ['spitter', 'stalker'] as const) {
