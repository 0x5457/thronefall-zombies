...
try {
  ...
  assert.equal((await page.evaluate(() => window.__pinefall.machine)).phase, 'day', 'game input is locked on the title screen');
  ...
  assert.equal(await page.evaluate(() => localStorage.getItem('pinefall.campaign.current') !== null), true, 'a new campaign writes a day-1 checkpoint');
  ...
  assert.match(await page.locator('#home-start span').textContent(), /再点一次/, 'first click only warns');
  assert.equal(await page.evaluate(() => window.__pinefall.home.visible), true, 'still on the title screen after the first click');
  assert.equal(await page.evaluate(() => localStorage.getItem('pinefall.campaign.current')), before, 'progress untouched before confirmation');
  await page.click('#home-start');
  await page.waitForFunction(() => !window.__pinefall.home.visible);
  ...
  console.log('PASS: title screen locks input, warns before overwrite, starts day 1 and renders on mobile');
