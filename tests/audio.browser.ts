...
import type { MusicScene, ScoreBus, Synth, Track } from '../src/audio.js';

interface OfflineAudioModule {
  TRACKS: Record<MusicScene, Track>;
  createSynth: (ctx: BaseAudioContext, maxVoices?: number) => Synth;
  createScoreBus: (ctx: BaseAudioContext, output: AudioNode, scene: MusicScene) => ScoreBus;
  musicStep: (
    synth: Synth,
    bus: ScoreBus,
    phase: MusicScene,
  ...
  playEffect: (synth: Synth, output: AudioNode, name: string, at: number) => void;
}

interface OfflineRender {
  phase: string;
  seconds: number;
  peak: number;
  rms: number;
...
try {
  ...
  const result = await page.evaluate(async () => {
    const mainScript = (document.querySelector('script[src*="main.ts"]') as HTMLScriptElement).src;
    const audioModule = (await import(
      new URL('audio.ts', new URL('./', mainScript)).href
    )) as OfflineAudioModule;
    const { TRACKS, createSynth, createScoreBus, musicStep, EFFECTS, playEffect } = audioModule;
    const outputs: OfflineRender[] = [];
    for (const phase of ['day', 'night', 'interior', 'sfx'] as const) {
      const rate = 22050,
        seconds =
          phase === 'sfx'
      ...
      const synth = createSynth(ctx, 10000),
        bus = createScoreBus(ctx, gain, phase as MusicScene);
      if (phase === 'sfx')
