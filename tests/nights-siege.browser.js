...
try {
  ...
  async function perk(id) {
    ...
    assert.ok(rating && rating.score >= 0 && rating.score <= 100 && ['S', 'A', 'B', 'C'].includes(rating.grade));
  ...
  await build('tower', -2, -8); await build('tower', 8, 2);
  ...
  for (let i = 0; i < 500; i++) {
    const s = await game();
    max.sieging = Math.max(max.sieging, s.sieging); max.spits = Math.max(max.spits, s.spits); max.hidden = Math.max(max.hidden, s.hidden);
    if (!shot && s.sieging > 0 && s.spits > 0) { await page.screenshot({ path: 'artifacts/nights-siege.png' }); shot = true; }
    if (s.over || s.perkPending) break;
    await page.waitForTimeout(400);
  }
  const end = await game();
  console.log('night 3 result', { max, day: end.day, health: end.health, over: end.over, kills: end.kills, deaths: end.deaths });
