...
test('furniture slots toggle with ownership and locked function slots stay dim', () => {
  ...
  room.setSlots(2);
  assert.equal(room.markers.workbench.material.opacity, .6, 'first slot open');
  assert.equal(room.markers.radio.material.opacity, .6, 'second slot open');
  assert.equal(room.markers.medcab.material.opacity, .2, 'third function slot still locked');
  assert.equal(room.markers.plant.material.opacity, .65, 'decor markers are never locked');
  room.setFurniture([]);
