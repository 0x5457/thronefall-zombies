...
try {
  ...
  async function walkTo(tx, tz) {
    for (let i = 0; i < 250; i++) {
      ...
      if (Math.hypot(tx - x, tz - z) < 0.5) return;
      const held = [];
      if (Math.abs(tx - x) > 0.25) held.push(tx > x ? 'd' : 'a');
      if (Math.abs(tz - z) > 0.25) held.push(tz > z ? 's' : 'w');
      for (const key of held) await page.keyboard.down(key);
      await page.waitForTimeout(80);
      for (const key of held) await page.keyboard.up(key);
    }
    throw new Error(
      `Unable to reach ${tx}, ${tz}: ${JSON.stringify(await page.evaluate(() => window.__pinefall.stats))}`,
    );
  }
  await walkTo(-7, -6.1);
  await walkTo(-12, 2.7);
  for (let x = -16; x >= -56; x -= 4) await walkTo(x, 6 + x * 0.19 + Math.sin(x * 0.13));
