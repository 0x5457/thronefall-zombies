...
try {
  ...
  await walk(5.8, -6.1); await walk(5.8, 1.5); await walk(0, 1.3);
  ...
  await page.keyboard.press('e');
  await page.waitForFunction(() => window.__pinefall.stats.transitioning && window.__pinefall.stats.viewZoom > 1.02);
  const transitionBudget = await page.evaluate(() => window.__pinefall.state.elapsed);
  await page.keyboard.press('e'); await page.keyboard.press('n'); await page.keyboard.press('2');
  await page.screenshot({ path: 'artifacts/rv-enter-transition.png' });
  await page.waitForFunction(() => window.__pinefall.stats.inside && !window.__pinefall.stats.transitioning);
  assert.equal(await page.evaluate(() => window.__pinefall.state.elapsed), transitionBudget, 'transition freezes budget');
  assert.equal((await stats()).viewZoom, 1, 'camera settles at original zoom');
  await page.waitForFunction(() => window.__pinefall.audio.phase === 'interior', null, { timeout: 10000 });
  ...
  await page.keyboard.press('n'); await page.keyboard.press('2');
  ...
  assert.ok((await stats()).player[2] >= .12, 'cannot walk into furniture');
  ...
  await page.keyboard.press('h');
  await page.locator('#rv-lamp').focus(); await page.keyboard.press('Space');
  ...
  assert.equal(await page.locator('#rv-lamp').getAttribute('aria-pressed'), 'false', 'pause blocks lamp interaction');
  ...
  await page.waitForFunction(() => window.__pinefall.audio.phase === 'day', null, { timeout: 10000 });
  assert.equal(await page.evaluate(() => window.__pinefall.audio.context), 'running');
  await page.emulateMedia({ reducedMotion: 'reduce' });
  await page.locator('#rv-door').click();
  assert.equal((await stats()).transitioning, false, 'reduced motion skips camera travel');
  await page.waitForFunction(() => window.__pinefall.stats.inside && !window.__pinefall.stats.transitioning);
  assert.equal(await page.locator('#rv-lamp').getAttribute('aria-pressed'), 'false', 'lamp choice survives exit/reentry');
  ...
  console.log('PASS: RV door, interior music active, outdoor music restored, movement, budget freeze, input guards, return, narrow viewport, night denial');
