...
try {
  const page = await browser.newPage({ viewport: { width: 1404, height: 1080 }, reducedMotion: 'reduce' });
  const errors = [];
  ...
  await page.route('**/src/main.js*', async route => {
    ...
    await route.fulfill({ response, body: effectEnabled ? body : body.replace('addOcclusionSilhouette(player);', '') });
  });
  const load = async () => {
    await page.goto('http://localhost:5173/thronefall-zombies/');
  ...
  await load(); await approachRV(); await page.keyboard.press('p');
  ...
  assert.ok(Math.abs(before[2] - after[2]) < .26, 'same RV-side location');
  await page.screenshot({ path: 'artifacts/occlusion-day.png' });
  await page.keyboard.press('p'); await page.keyboard.press('n');
  await page.waitForTimeout(9000); await page.keyboard.press('p');
  ...
  const pixels = await page.evaluate(async () => {
    ...
    const mainURL = new URL(document.querySelector('script[src*="src/main.js"]').src);
    ...
    const scene = new T.Scene(); scene.background = new T.Color('#304035');
    scene.add(new T.AmbientLight(0xffffff, 2));
    const camera = new T.OrthographicCamera(-1.2, 1.2, 2, -.4, .1, 20);
    camera.position.set(0, 0, 8); camera.lookAt(0, 0, 0);
    const player = character(); scene.add(player); addOcclusionSilhouette(player);
    player.userData.legs[0].rotation.x = .6;
    const overlays = [];
    player.traverse(o => { if (o.name === 'occlusion-silhouette') overlays.push(o); });
    const wall = new T.Mesh(new T.BoxGeometry(3, 3, .2), new T.MeshBasicMaterial({ color: '#937857' }));
    wall.position.set(0, .8, 2); scene.add(wall);
    function draw(enabled) {
      overlays.forEach(o => { o.visible = enabled; });
    ...
    function diff() {
      const a = draw(false), b = draw(true); let left = 0, right = 0;
      const colors = new Set();
      for (let i = 0; i < a.length; i += 4) {
        if (a[i] !== b[i] || a[i + 1] !== b[i + 1] || a[i + 2] !== b[i + 2]) {
          if ((i / 4) % 128 < 64) left++; else right++;
          colors.add(`${b[i]},${b[i + 1]},${b[i + 2]}`);
        }
      }
      return { left, right, colors: colors.size };
    }
    wall.visible = false; const clear = diff();
    wall.visible = true; const full = diff();
    wall.position.x = -1.5; const partial = diff();
    wall.position.z = -2; const behind = diff();
    // Stencil resets between frames, so moving back into the open leaves no trace.
    wall.visible = false; const restored = diff();
  ...
  assert.equal(pixels.behind.left + pixels.behind.right, 0, 'wall behind player does not highlight');
