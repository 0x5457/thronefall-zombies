...
try {
  ...
  assert.equal((await audio()).music, .44);
  ...
  await page.waitForTimeout(400); assert.equal((await audio()).step, paused.step);
  ...
  for (const [x,y] of [[720,390],[640,380],[820,390],[850,500]]) {
    await page.mouse.move(x,y); await page.waitForTimeout(120);
    if (!(await page.locator('#world-label').getAttribute('class')).includes('invalid')) { await page.mouse.click(x,y); break; }
  }
  assert.equal((await audio()).counts.build, 1);
  await page.keyboard.press('n');
  await page.waitForFunction(() => window.__pinefall.audio.phase === 'night', null, { timeout: 10000 });
  ...
  await page.reload(); await page.waitForSelector('#loading.done', { timeout: 60000 });
  await page.click('#photo');
  assert.equal((await audio()).context, 'locked', 'saved mute survives other gestures');
  assert.equal((await audio()).music, .44);
  await page.keyboard.press('h'); await page.click('#sound');
  await page.waitForFunction(() => window.__pinefall.audio.context === 'running');
  ...
  const result = await page.evaluate(async () => {
    ...
    for (const phase of ['day','night','interior','sfx']) {
      ...
      const gain = ctx.createGain(); gain.gain.value = .65 * .65; gain.connect(ctx.destination);
      // Offline scheduling enqueues the entire score at once; realtime caps only the look-ahead window.
      const synth = createSynth(ctx, 10000), bus = createScoreBus(ctx, gain, phase);
      if (phase === 'sfx') Object.keys(EFFECTS).forEach((name,i) => playEffect(synth, gain, name, i * 1.4));
      else for (let i = 0; i < 256; i++) musicStep(synth, bus, phase, i, i * 30 / TRACKS[phase].bpm, phase === 'night' && i >= 128 ? .85 : 0);
      const buffer = await ctx.startRendering();
      const left = buffer.getChannelData(0), right = buffer.getChannelData(1);
      let peak = 0, sum = 0, stereo = 0;
      ...
      for (let i=0;i<left.length;i++) {
        peak=Math.max(peak,Math.abs(left[i]),Math.abs(right[i])); sum+=left[i]**2; stereo+=Math.abs(left[i]-right[i]);
      ...
      outputs.push({ phase, seconds, peak, rms:Math.sqrt(sum/left.length), stereo:stereo/left.length, voices:synth.voices, wav:btoa(binary) });
    }
    return outputs;
  });
  for (const output of result) {
    ...
    assert.ok(output.peak > .01 && output.peak < .95, `${output.phase} non-silent, headroom before compressor`);
    assert.ok(output.rms > .001);
    if (output.phase !== 'sfx') assert.ok(output.stereo > .001, 'stereo layers differ');
    delete output.wav;
  }
