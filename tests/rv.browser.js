...
try {
  ...
  assert.equal(
    await page.locator('[data-rv-install="radio"]').isDisabled(),
    true,
  ...
  await page.waitForFunction(() => !document.querySelector('#radio-alert').hidden, null, {
    timeout: 15000,
  });
  assert.match(await page.locator('#radio-alert').textContent(), /下一路 北径/);
  await page.waitForFunction(() => window.__pinefall.stats.windowGlow > 0.2, null, {
    timeout: 15000,
  });
  ...
  console.log(
    'PASS: RV slots force a choice, mods alter the real weapon, radio sells timeline + night alert, windows glow',
  );
} finally {
  await browser.close();
}
