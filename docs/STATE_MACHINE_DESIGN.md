...
> 状态：设计已确认（2026-09-19）；ARC-01..04 已实现并标 `VERIFY`（见 §7），进度以 `DEVELOPMENT.md` 与代码/测试证据为准。
> 规则数值仍以 `src/rules.ts` 为运行时唯一来源；本文定义状态边界、流转与存档格式。
> 依赖决策：引入 `xstate@5.33.2`，理由与代价见本文 §2，决策记录见 `GAME_DESIGN.md` §10。

## 1. 背景与问题

现有权威数据已部分集中：`src/rules.ts` 的 `newGame()` 是初始状态，`buy/advance/choosePerk/expedition/upgrade/...` 是带合法性校验的纯函数，等价于 reducer。真正的缺口：

1. **权威数据不完整**：建筑在 `src/main.ts` 的 `place()` 建造流程里是「数据 + Three.js mesh」混合对象（`mesh/light/growth/hit`），采集节点 `world.logs` 不在状态里，存档无从序列化。
2. **阶段是隐式状态机**：`state.phase` + `inside` + `paused` + `over` + `perkPending` 多个标志拼出流程，非法组合只能靠各处 `if` 兜底，没有单一事实来源。
3. **无序列化与版本迁移**：T6（黎明检查点存档）仍是 TODO（现已由 ARC-04 落地，见 §7），设计文档 §9 已定好存档边界与格式意图。
...
- 代价：新增运行时依赖、构建体积增大（已实现，构建结果与限制见 `GAME_DESIGN.md` §10 决策记录）；现有 `state.phase/paused` 读取点需分批迁移，本轮采用「保留 `state` 引用、增量替换」策略。
- 实现约束：不使用 `assign` 重建 context，避免破坏 `rules.ts` 依赖的 `state` 对象引用；machine actions 直接调用规则函数修改该对象。`machine.ts` 只导入 `setup/createMachine/createActor`。
...
| 事件 | 触发点 | guard | action（调用 rules.ts 纯函数） |
|---|---|---|---|
...
1. **context 复用现有 `state` 对象**：`rules.ts` 的函数签名与数值一行不改，作为 guards/actions 被 machine 调用；避免大规模重写与规则回归。
2. **单一写入者**：`state.phase` 只允许由 machine action 修改（经由 `advance` 等函数）；machine 的状态值与 `state.phase` 必须一致。`machine.ts` 导出 `assertPhaseSync(snapshot)`，`startCampaign()` 启动时即校验，单测在全流程每步校验。
...
- **入口**：启动加载完成后进入首页（`src/home.ts`）：开始新游戏 / 继续游戏（显示第 N 天 · 营地 % · 专长数）；已有存档时开始新游戏需二次点击确认覆盖。首页背景使用真实营地的标题镜头（黄昏、环绕、点击篝火溅火星），webdriver 自动化默认跳过首页，`?home=1` 强制显示以便测试。
...
| ID | 任务 | 验收 |
|---|---|---|
| ARC-01 | `src/machine.ts` + `tests/machine.test.ts`：状态图、事件、guards/actions 骨架 | 合法/非法转移、与 `rules.ts` 的 phase 同步均有 Node 单测；不接 `main.ts`、不改行为 |
| ARC-02 | `main.ts` 接入 actor：布尔标志迁移到 `snapshot.matches` | 现有 campaign/combat/interior/nights 浏览器回归全过，行为不变 |
| ARC-03 | 权威数据化：buildings/logs/进度进入 `state`，场景改 id→mesh 注册表 | 状态 JSON 往返一致；建造/升级/拆除/读档重建浏览器验证 |
| ARC-04 | `src/save.ts` + 黎明检查点 + 继续入口 | 守到黎明→写入→刷新→继续，日数/资源/专长/建筑/倒木一致；坏档回退、高版本拒绝且不覆写 |

ARC-01..04 均已实现并标 `VERIFY`（待用户实机验收后转 `DONE`），T6 随 ARC-04 落地：

- ARC-01：`src/machine.ts` + `tests/machine.test.ts` 8 项与 `tests/machine-graph.test.ts` 2 项；`startCampaign()` 启动即 `assertPhaseSync`。
- ARC-02：`main.ts` 事件化（`START_NIGHT/CLEARED/CHOOSE_PERK/ENTER_RV/EXIT_RV/PAUSE/RESUME/OPEN_*/CLOSE_*/CAMP_DESTROYED`），`state.phase/paused` 由 machine 单一写入；campaign/combat/nights/nights-siege/feel/interior/rv 浏览器回归全过。
- ARC-03：`state.buildings`（纯数据 + id）与 `state.logs`（稳定 id），场景 `Map<id,{mesh,light}>`；save 往返与读档重建由 `tests/save.*` 验证。
- ARC-04：`src/save.ts`（双槽、损坏回退、容量/版本处理）+ `tests/save.test.ts` 6 项 + `tests/save.browser.ts`（真实黎明写入、首页续玩、坏档回退、高版本拒绝）+ `tests/home.browser.ts`（首页锁定输入、覆盖确认、移动端）。
...
- 图模型（`@xstate/graph`，仅 devDependency）：`toDirectedGraph` 冻结 phase/overlay 节点与全部边；`getAdjacencyMap` 以真实场景快照为起点，冻结 9 个场景 × 全部事件类型的落点（含拒绝），并逐事件在真机 actor 上复验。因 context 可变且 guards 依赖非 assign action 的副作用，不使用 `getShortestPaths`/`TestModel` 路径回放：路径模拟不执行这些 action，会产出真机拒绝的路径。
- 浏览器：更新 `tests/campaign.browser.ts` 走完整 day→night→dawn 并以 machine 状态断言；新增 `tests/save.browser.ts`（黎明写入、刷新续玩、坏档回退）；其余 `tests/*.browser.ts` 作为回归。
- 不测真实 GPU 帧率；不把 headless 结果写成性能达标。

## 9. 风险与未决

- 体积：xstate 进入应用 bundle 后由构建产物记录增量；现有 >500 kB 警告不得被掩盖。
- 双写 phase：靠「单一写入者」规则与 ARC-01 同步断言控制；`advance()` 未拆分，职责仍由 rules.ts 承担。
